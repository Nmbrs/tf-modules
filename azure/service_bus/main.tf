resource "azurerm_servicebus_namespace" "main" {
  ...
  dynamic "network_rule_set" {
    ...
    content {
      default_action                = length(var.firewall_settings.allowed_subnet_ids) > 0 ? "Deny" : "Allow"
      public_network_access_enabled = var.firewall_settings.public_network_access_enabled
      trusted_services_allowed      = var.firewall_settings.trusted_services_bypass_firewall_enabled

      dynamic "network_rules" {
        for_each = var.firewall_settings.allowed_subnet_ids
        content {
  ...
  lifecycle {
    ...
    ## firewall_settings.allowed_subnet_ids validation — Premium SKU required
    precondition {
