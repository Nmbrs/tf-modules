locals {
  private_endpoint_name   = "pep-${var.resource_settings.name}-${lower(local.subresource_name[var.resource_settings.type])}"
  service_connection_name = "sc-${var.resource_settings.name}-${lower(local.subresource_name[var.resource_settings.type])}"
  ...
  resource_data_blocks = {
    ...
    data_factory_portal   = data.azurerm_data_factory.data_factory,
